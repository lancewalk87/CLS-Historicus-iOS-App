...
-(void)Begining{ //Inital Void
    ...
    Unit4Scroll.hidden = YES; Unit5Scroll.hidden = YES; FlashCardTitle.hidden = YES; AnswertocardLabel.hidden = YES; Unit1Button.hidden = YES;
...
-(void)PauseVoid{
    if (SearchBOOL == YES){
        ...
        //GUI
        ReturnButton.hidden = YES; NextCard.hidden = YES; AnswertocardLabel.hidden = YES; Answertocard.hidden = YES; QuestionNumber.hidden = YES;
        
        QuitButton.hidden = NO; SearchBar.hidden = NO; SearchButton.hidden = NO; PreviousCard.hidden = YES; ResumeButton.hidden = NO; FlashCardTitle.hidden = YES;
...
-(void)CycleVoid{ //Search Void == Response
    ...
    if ([ReturnString containsString:EndStringUpper]){
        [self SearchResultVoid];
        
    } else if ([ReturnString containsString:EndStringLower]){
        [self SearchResultVoid];
    }
...
-(void)SearchResultVoid{
    //Boolien Control
    NoResults = NO; NextResultBOOL = YES;
    //End Boolien Control
...
-(void)BeginSearch{
    if ([SearchBar.text  isEqual: @""]){
        ...
        //Needs to eliminate all spaces to the point that spaces are not subject to effect the results returned from the user's inital search string
        //End Integer Control
        [Answertocard setTitle:@"Reveal Answer" forState:UIControlStateNormal];
        //UI Element Control
        //Step 1:
        ...
        Cycletimer = [NSTimer scheduledTimerWithTimeInterval:0.00001 target:self selector:@selector(CycleVoid) userInfo:nil repeats:YES];
        Timer = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(SecondsVoid) userInfo:nil repeats:YES];
...
-(IBAction)ResumeButton:(id)sender{ //Resume Buttons Remains the same for every menu selection
    [self performSelector:@selector(ResumeVoid) withObject:nil afterDelay:0.01];
}
...
-(IBAction)FlashCardNext:(id)sender{ //Next Term : Next Notes
    if (FlashCards == YES){
        if (NextResultBOOL == YES){
            SearchBOOL = YES; NextSelect = YES; PreviousSelect = NO;
            //GUI
...
-(IBAction)PreviousCard:(id)sender{ //Previous Term
    if (FlashCards == YES){
        if (NextResultBOOL == YES){
            SearchBOOL = YES; PreviousSelect = YES; NextSelect = NO;
            //GUI
            ResumeButton.hidden = YES; QuitButton.hidden = YES; SearchButton.hidden = YES;
            //End GUI
